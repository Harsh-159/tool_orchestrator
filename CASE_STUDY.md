...
- `GET /state` — read-only snapshot of all mock state; the evaluator reads it after a run to verify end-state.
- `POST /chat` — **stub. This is what you implement.**
- `evaluator/` — the scoring harness we will run against your `/chat` endpoint (see *How you're scored* below).
...
We run an **automated evaluator** (`evaluator/run.py`) against your `/chat` endpoint. It posts ~30 scenarios — single-service lookups, cross-service joins, multi-step plans, ambiguous prompts where you should ask, and error paths where you should not fabricate success. State is reset between scenarios.
